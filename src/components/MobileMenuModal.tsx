...
const MobileMenuModal: React.FC<isModalOpenProps> = ({isModalOpen, setIsModalOpen}) => {
  ...
  useEffect(() => {
    if (isModalOpen) {
      ...
      setIsModalFadeIn(false)
    }
  })

  return (
    <div onMouseDown={handleClickOutside} className={`ease-in-out duration-500 ${isModalFadeIn? 'translate-x-0': 'translate-x-full'}`}>
      <div ref={modalRef} className='bg-darkByzantineBlue mx-6 px-6 rounded-[10px] h-[383px] flex flex-col items-center justify-center gap-7'>
        <div>
          <Navbar isModal={true}/>
        </div>
